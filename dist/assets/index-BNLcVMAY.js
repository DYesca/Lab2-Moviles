...
function FlatpickrInstance(element, instanceConfig) {
  ...
  function setHoursFromDate(dateObj) {
    var date2 = dateObj || self.latestSelectedDateObj;
  ...
  function createDay(className, date2, _dayNumber, i) {
    ...
    dayElement.$i = i;
    dayElement.setAttribute("aria-label", self.formatDate(date2, self.config.ariaDateFormat));
    if (className.indexOf("hidden") === -1 && compareDates(date2, self.now) === 0) {
    ...
    if (dateIsEnabled) {
      ...
      if (isDateSelected(date2)) {
        ...
        if (self.config.mode === "range") {
          toggleClass(dayElement, "startRange", self.selectedDates[0] && compareDates(date2, self.selectedDates[0], true) === 0);
          toggleClass(dayElement, "endRange", self.selectedDates[1] && compareDates(date2, self.selectedDates[1], true) === 0);
    ...
    if (self.weekNumbers && self.config.showMonths === 1 && className !== "prevMonthDay" && i % 7 === 6) {
      self.weekNumbers.insertAdjacentHTML("beforeend", "<span class='flatpickr-day'>" + self.config.getWeek(date2) + "</span>");
    }
  ...
  function isEnabled(date2, timeless) {
    ...
    var dateToCheck = self.parseDate(date2, void 0, timeless);
  ...
  function minMaxDateSetter(type) {
    return function(date2) {
      var dateObj = self.config["_" + type + "Date"] = self.parseDate(date2, self.config.dateFormat);
    ...
    if (inputDate instanceof Array)
      ...
      switch (self.config.mode) {
        ...
        case "time":
          ...
          break;
        case "multiple":
          dates = inputDate.split(self.config.conjunction).map(function(date2) {
            return self.parseDate(date2, format);
          });
          break;
        case "range":
          dates = inputDate.split(self.l10n.rangeSeparator).map(function(date2) {
            return self.parseDate(date2, format);
          });
          break;
      }
  ...
  function isDateInRange(date2) {
    if (self.config.mode !== "range" || self.selectedDates.length < 2)
      return false;
    return compareDates(date2, self.selectedDates[0]) >= 0 && compareDates(date2, self.selectedDates[1]) <= 0;
  }
...
btn.addEventListener("click", () => {
  const date2 = new Date(document.getElementById("dateInput").value).toISOString();
  const today = (/* @__PURE__ */ new Date()).toISOString();
  const months = diffMonths(today, date2);
  response.innerHTML = `You are <Strong>
                ${Math.floor(months / 12)} years and 
                ${months % 12} months old</Strong>`;
});
